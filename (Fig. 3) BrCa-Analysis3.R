...
EBrCa <- apply(BC.RNAseq.E[,c(3:ncol(BC.RNAseq.E))]/(BC.RNAseq.E[,c(3:ncol(BC.RNAseq.E))] + 2^Esig$KD), 2, mean)

pdf("BrCa-Epi-Genes-Distribution.pdf", width = 7, height = 7)
opar <- par(mfrow = c(3,3))
for (i in 1:length(Esig$Genes))
{
  values <- as.numeric(BC.RNAseq.EMT[match(Esig$Genes[i], BC.RNAseq.EMT$Description), c(3:ncol(BC.RNAseq.EMT))])
  plot(density(log2(values + 0.001), adj = 0.25), xlab = "Expression (log2 RPKM)", 
       main = list(cex = 0.8, paste("Gene:", Esig$Genes[i])))
  lines(c(Esig$KD[i], Esig$KD[i]), c(0,1), lty = 2, col = "red")

  xval <- as.numeric(BC.RNAseq.EMT[BC.RNAseq.EMT$Description == "WISP1"  , c(3:ncol(BC.RNAseq.EMT))])
  yval <- as.numeric(BC.RNAseq.EMT[match(Esig$Genes[i], BC.RNAseq.EMT$Description), c(3:ncol(BC.RNAseq.EMT))])
  WH <- xval > 0.81
  WLGH <- length(yval[yval > 0.81 & !WH])
  WLGL <- length(yval[yval <= 0.81 & !WH])
  WHGH <- length(yval[yval > 0.81 & WH])
  ...
  Ftest <- fisher.test(matrix(c(WLGH, WHGH, WLGL, WHGL), nrow = 2))
  plot(log2(xval + 0.001), log2(yval + 0.001), xlim = c(-10,6), ylim = c(-10,10), xlab = "WISP1 (log2 RPKM)", 
       main = list(cex = 0.8, col = "black", paste("EMT:", Esig$Genes[i], " p-value =", sprintf("%6.4f", Ftest$p.value))))
  #,"; P-val=", sprintf("%6.4f", cor.test(xval, yval)$p.value)), col = c("red", "black", "black", "black", "black", "blue")[as.factor(Site)])
  lines(log2(c(1, 1)), c(-10,14), col = "blue", lty = 2)
  lines(c(-10,6), log2(c(1, 1)), col = "blue", lty = 2)
}
...
Msig <- data.frame(Genes = tmp[,1], KD = as.numeric(tmp[,2]))
...
MBrCa <- apply(BC.RNAseq.M[,c(3:ncol(BC.RNAseq.M))]/(BC.RNAseq.M[,c(3:ncol(BC.RNAseq.M))] + 2^Msig$KD), 2, mean)

pdf("BrCa-Mesen-Genes-Distribution.pdf", width = 7, height = 7)
opar <- par(mfrow = c(3,3))
for (i in 1:length(Msig$Genes))
{
  values <- as.numeric(BC.RNAseq.EMT[match(Msig$Genes[i], BC.RNAseq.EMT$Description), c(3:ncol(BC.RNAseq.EMT))])
  plot(density(log2(values + 0.001), adj = 0.25), xlab = "Expression (log2 RPKM)", 
     main = list(cex = 0.8, paste("Gene:", Msig$Genes[i])))
  lines(c(Msig$KD[i], Msig$KD[i]), c(0,1), lty = 2, col = "red")

  xval <- as.numeric(BC.RNAseq.EMT[BC.RNAseq.EMT$Description == "WISP1"  , c(3:ncol(BC.RNAseq.EMT))])
  yval <- as.numeric(BC.RNAseq.EMT[match(Msig$Genes[i], BC.RNAseq.EMT$Description), c(3:ncol(BC.RNAseq.EMT))])
  WH <- xval > 0.81
  WLGH <- length(yval[yval > 0.81 & !WH])
  WLGL <- length(yval[yval <= 0.81 & !WH])
  WHGH <- length(yval[yval > 0.81 & WH])
  ...
  Ftest <- fisher.test(matrix(c(WLGH, WHGH, WLGL, WHGL), nrow = 2))
  plot(log2(xval + 0.001), log2(yval + 0.001), xlim = c(-10,6), ylim = c(-10,10), xlab = "WISP1 (log2 RPKM)", 
       main = list(cex = 0.8, col = "black", paste("EMT:", Msig$Genes[i], " p-value =", sprintf("%6.4f", Ftest$p.value))))
  #,"; P-val=", sprintf("%6.4f", cor.test(xval, yval)$p.value)), col = c("red", "black", "black", "black", "black", "blue")[as.factor(Site)])
  lines(log2(c(1, 1)), c(-10,14), col = "blue", lty = 2)
  lines(c(-10,6), log2(c(1, 1)), col = "blue", lty = 2)
}
...
write.csv(BrCa_State, file = "BrCa-EMT-State-March20.csv")

BrCa_EMT <- read.csv(file = "BrCa-EMT-State-March20.csv")
##################################################################
#
#
#
...
pdf("PCA-analysis-BrCa-CellLinesProjection-March20.pdf", width = 25, height = 25)
plot(log2(BrCa_EMT$Mesenchymal + 0.0001), log2(BrCa_EMT$Epithelial + 0.0001), type = "p", pch = 19, col = ColorDot, 
     xlab = "Mesenchymal Signal", ylab = "Epithelial Signal", xlim = c(-4, 0), ylim = c(-4.5,0))
lines(log2(xvals+0.0001), log2(yvals+0.0001), lty = 2, col = "black")
text(log2(BrCa_EMT$Mesenchymal + 0.0001), log2(BrCa_EMT$Epithelial + 0.0001)+0.05, labels = SlimCL, cex = 0.5)

#legend(x="bottomleft", legend = c("Basal", "Claudin Low", "HER2", "Luminal A", "Luminal B"), col = c("red", "yellow", "pink", "blue", "black"), pch = 19, cex = 1)

plot(log2(BrCa_EMT$Mesenchymal + 0.0001), log2(BrCa_EMT$Epithelial + 0.0001), type = "p", pch = 19, col = ColorDot, 
     xlab = "Mesenchymal Signal", ylab = "Epithelial Signal", xlim = c(-3.5, -0.5), ylim = c(-1,0))
lines(log2(xvals+0.0001), log2(yvals+0.0001), lty = 2, col = "black")
text(log2(BrCa_EMT$Mesenchymal + 0.0001), log2(BrCa_EMT$Epithelial + 0.0001)+0.05, labels = SlimCL, cex = 0.5)

plot(log2(BrCa_EMT$Mesenchymal + 0.0001), log2(BrCa_EMT$Epithelial + 0.0001), type = "p", pch = 19, col = ColorDot, 
     xlab = "Mesenchymal Signal", ylab = "Epithelial Signal", xlim = c(-1.3, 0), ylim = c(-4,0))
lines(log2(xvals+0.0001), log2(yvals+0.0001), lty = 2, col = "black")
text(log2(BrCa_EMT$Mesenchymal + 0.0001), log2(BrCa_EMT$Epithelial + 0.0001)+0.05, labels = SlimCL, cex = 0.5)
